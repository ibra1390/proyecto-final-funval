...
export function initLocations() {
    ...
    locationSearch.addEventListener("focus", () =>{
        ...
        //Close guests dropdown if open
        document.querySelector("#guests-dropdown").classList.add("hidden");
    });
    ...
    document.addEventListener("click", (e) => {
        if (!e.target.closest("#location-search") && !e.target.closest("#location-suggestions")) {
            locationSuggestions.classList.add("hidden"); //hide the suggestions dropdown
        }
    });
    ...
    locationSuggestions.addEventListener("click", (e) => {
        ...
        if (clickedSuggestion) {
            ...
            //location displays in searchbar
...
function showSuggestions(locations, container) {
    ...
    if (locations.length === 0) {
        container.innerHTML = `<div class="p-4 text-center text-gray-500 dark:text-white text-sm font-montserrat">No locations found</div>`;
    } else {
        locations.forEach(location => {
            container.innerHTML += `
                <div class="flex items-center gap-3 p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600 location-suggestion" 
                    data-location="${location}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 text-gray-500 dark:text-gray-200">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" />
                    </svg>
                    <span class="text-sm font-montserrat text-gray-500 dark:text-gray-100">${location}</span>
                </div>
            `;
        });
    }
...
function updateLocation(location) {
    ...
    if (locationText) {
       ...
       locationText.classList.add("text-gray-800", "dark:text-gray-200");
    }
}
